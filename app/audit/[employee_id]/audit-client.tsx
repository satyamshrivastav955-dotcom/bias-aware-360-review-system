...
export default function AuditClient({ employee }: { employee: Employee }) {
  ...
  return (
    <div className="app-shell">
      ...
      <nav className="mb-6 border-b border-slate-200 dark:border-slate-800 pb-4 text-xs font-semibold text-slate-500 dark:text-slate-400">
        <Link href={`/review/${id}`} className="hover:text-slate-900 dark:hover:text-slate-100 transition-colors">
      ...
      <header className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#20272e] p-6 shadow-xs">
        ...
        {rows?.length ? (
          <p className="mt-4 font-mono text-[10px] uppercase tracking-[0.12em] text-slate-500 dark:text-slate-400">
            {source === "server"
      ...
      {rows === null ? null : rows.length === 0 ? (
        <div className="mt-8 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#20272e] p-12 text-center shadow-xs">
        ...
        <ol className="mt-8 divide-y divide-slate-200 dark:divide-slate-800 border-t border-slate-200 dark:border-slate-800">
          {rows.map((r, i) => (
            <li key={i} className="py-6">
              <div className="flex flex-wrap items-baseline gap-x-6 gap-y-2">
                ...
                <span className="text-sm font-medium text-slate-900 dark:text-slate-100">{r.who}</span>
                <span className="text-sm text-slate-600 dark:text-slate-300">{r.summary}</span>
              </div>

              {r.edits.map((e) => (
                <div key={e.point_ref} className="mt-4 border-l-2 border-slate-300 dark:border-slate-700 pl-4">
                  <p className="font-mono text-xs uppercase tracking-wider font-semibold text-slate-500 dark:text-slate-400">
                    {e.point_ref}
                  </p>
                  <p className="mt-1.5 text-xs leading-relaxed text-slate-400 line-through">
                    {e.before}
                  </p>
                  <p className="mt-1.5 text-sm leading-relaxed text-slate-800 dark:text-slate-200">{e.after}</p>
                </div>
              ))}

              {r.acknowledged.length > 0 && (
                <p className="mt-3 font-mono text-xs uppercase tracking-wider text-slate-500 dark:text-slate-400">
